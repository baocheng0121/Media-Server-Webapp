...
def single_song(song_id):
    ...
    artworkmetadata = database.get_SongArtwork(song_id)
    descriptionmetadata = database.get_SongDescription(song_id)
    # Data integrity checks
    if song == None:
        song = []
...
def add_song():
    ...
    if request.method == 'POST':
        ...
        if ('storage_location' not in request.form):
            ...
            song_params[0] = request.form['storage_location']
        if ('description' not in request.form):
            song_params[1] = 'Empty Song description'
        else:
            song_params[1] = request.form['description']
        if ('song_title' not in request.form):
            song_params[2] = 'Empty Song Title'
        else:
            song_params[2] = request.form['song_title']
        if ('length' not in request.form):
            song_params[3] = '0'
        else:
            song_params[3] = request.form['length']

        if ('song_genre' not in request.form):
            song_params[4] = 'rock'
        else:
            song_params[4] = request.form['song_genre']
        if ('artist_id' not in request.form):
            song_params[5] = '1'
        else:
            song_params[5] = request.form['artist_id']
        # Once retrieved, do some data integrity checks on the data
        print("it is here")
        print(song_params)
        # Once verified, send the appropriate data to the database for insertion
        c = database.add_song_to_db(song_params)
        print(c)
        # NOTE :: YOU WILL NEED TO MODIFY THIS TO PASS THE APPROPRIATE VARIABLES
        return single_song(c)
        # return render_template('singleitems/song.html',
        #                    session=session,
        #                    page=page,
        #                    user=user_details)
...
##
# NEW Function : search song
# ##
@app.route('/search/songs', methods=['POST','GET'])
def search_songs():
    """
    ...
    # Get a list of matching artists from the database
    songs = None
    if(request.method == 'POST'):
...
##
# NEW Function : search artist
# ##
@app.route('/search/artists', methods=['POST','GET'])
def search_artists():
    """
    ...
    if(request.method == 'POST'):
        ...
        artists = database.find_matchingartists(request.form['searchterm'], request.form['searchterm'])
...
def search_albums():
    ...
    if(request.method == 'POST'):
        ...
        albums = database.find_matchingalbums(request.form['searchterm'], request.form['searchterm'])
...
def search_podcasts():
    ...
    if(request.method == 'POST'):
        ...
        podcasts = database.find_matchingpodcasts(request.form['searchterm'], request.form['searchterm'])
...
# NEW : media player
@app.route('/podcastep/<media_id>')
def play_single_podcastep(media_id):
    page['title'] = 'Podcast Player' # Add the title

    podcast = database.get_podcastep_uri(media_id)

    return render_template('mediaplayer/podcastplayer.html',
                           session=session,
                           page=page,
                           user=user_details,
                           podcast=podcast)
